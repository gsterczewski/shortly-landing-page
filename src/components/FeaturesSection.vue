<template>
  <section class="features-section">
    <form class="link-form">
      <input class="link-form-input" :class="isError ? 'invalid' :''" type="text" name="" v-model="url" />
      <span v-if="isError" class="link-form-error">{{ errorMessage }}</span>
      <span v-if="isPending">pending....</span>
      <button class="link-form-button" @click.prevent="submit" :disabled="isPending">
        Shorten It!
      </button>
    </form>
      <ul>
        <shortlink  v-for="link in links" :key="link.code" :link="link"/>
      </ul>
